...
object HotcellUtils {
  ...
  def checkneighbours(x: Long, y: Long, z: Long, neigh_x: Long, neigh_y: Long, neigh_z: Long): Boolean = {
    var ans = false
    if (neigh_x > maxX || neigh_x < minX || neigh_y < minY || neigh_y > maxY || neigh_z > maxZ || neigh_z < minZ) ans = false
    else if ((x - 1 <= neigh_x && neigh_x <= x + 1) && (y - 1 <= neigh_y && y + 1 >= neigh_y) && (z - 1 <= neigh_z && z + 1 >= neigh_z)) ans = true
    else ans = false
    ans
  }
  ...
  // need to write functions for calculating G values
